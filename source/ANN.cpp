...
Ann::Ann(int nNIL, int nNOL,int nNHL):numNodesInputLayer(nNIL), numNodesOutputLayer(nNOL),numNodesHiddenLayer(nNHL)
{
    ...
    this->FirstWeightMatrix = MatrixXd::Zero(this->InputLayer.size(),this->HiddenLayer.size()- 1).unaryExpr([](double dummy){return n_uniform(e);});
    this->SecondWeightMatrix = MatrixXd::Zero(this->HiddenLayer.size(),this->OutputLayer.size()).unaryExpr([](double dummy){return n_uniform(e);});
}
